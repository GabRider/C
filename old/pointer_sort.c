...
bool is_number(char* number)
{
  ...
  while(number[cnt] != '\0')
  {
    if ((int)number[cnt]<48 || (int)number[cnt]>57) {
      return false;
    }
    cnt++;
  }
...
int main(int argc,char** argv){
  int size=argc-1;
  int *no_sorted= (int*)malloc((size-1) *sizeof(int));
  int **sorted= (int**)malloc((size-1) *sizeof(int*));

  for (int i = 1; i < argc; i++) {
    if (is_number(argv[i])) {
      no_sorted[i-1]= atoi(argv[i]);
      sorted[i-1]= no_sorted+i;
    }
    else
    {
      exit(0);
  ...
  bullet_sort(sorted,size-1);
  print_table_sort(sorted,size-1 );
  free(no_sorted);
  free(sorted);
}
